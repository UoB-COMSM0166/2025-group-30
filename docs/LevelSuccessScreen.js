class LevelSuccessScreen extends Screen {
  constructor(screenManager, gameScreen) {
    ...
    this.winImage = loadImage("assets/farmer_win.gif");
  ...
  display() {
    ...
    text(
      `Level ${this.gameScreen.level.level} Complete!`,
      baseWidth / 2,
      baseHeight / 2 - 10
    );
    ...
    if (this.gameScreen === this.screenManager.single) {
      text(
        `Score: ${this.gameScreen.player.score}`,
        baseWidth / 2,
        baseHeight / 2 + 40
      );
      ...
      text(
        `Score: ${
          this.gameScreen.player1.score + this.gameScreen.player2.score
        }`,
        baseWidth / 2,
        baseHeight / 2 + 40
      );
    ...
    textSize(24);
    text(
      `Target: ${this.gameScreen.level.targetScores}`,
      baseWidth / 2,
      baseHeight / 2 + 80
    );
